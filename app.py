...
def predict_snippets(X_seq, window_size=100, step_size=50):
    ...
    for start in range(0, len(X_seq) - window_size + 1, step_size):
        ...
        results.append({
            ...
            # NEW: ensemble output
            "ensemble_pred": pred["ensemble"]["prediction"]
        })
...
if st.button("Run Detection") and sequence_array is not None:
    ...
    if y_sequence is not None:
        ...
        start_lines = alt.Chart(fall_intervals).mark_rule(color="red", strokeWidth=2).encode(x="start")
        end_lines = alt.Chart(fall_intervals).mark_rule(color="red", strokeWidth=2, strokeDash=[4,2]).encode(x="end")
    ...
    with col1:
        ...
        lstm_table = snippet_df[["start","end","lstm_prob","lstm_pred"]]
    ...
    with col2:
        ...
        rf_table = snippet_df[["start","end","rf_prob","rf_pred"]]
    ...
    with col3:
        ...
        hmm_table = snippet_df[["start","end","hmm_pred"]]
        st.markdown("Window-level predictions (HMM)")
        st.dataframe(hmm_table)


    # ----------------------------
    # FINAL ALERT (ENSEMBLE)
    # ----------------------------
    st.markdown("---")
    st.subheader("🚨 Final Fall Alert (Ensemble Decision)")

    snippet_df["combined_alert"] = snippet_df["ensemble_pred"]

    combined_alert = snippet_df[snippet_df["combined_alert"]=="falling"]

    if len(combined_alert) > 0:
        st.error("⚠ Fall detected by ensemble system")

        st.dataframe(
            combined_alert[["start","end","lstm_pred","rf_pred","hmm_pred"]]
        )

    else:
        st.success("No fall detected by ensemble system.")
